...
function vk_send(int $peer_id, string $message, array $attachment = [], $forward_messages = '')
{
    ...
    if ($message != '' || !empty($forward_messages)) {
        ...
        if ($forward_messages) {
            $request['forward_messages'] = $forward_messages;
        }
...
function vk_api_call(string $method, array $params = [])
{
    ...
    if (isset($decoded_body['error'])) {
        throw new Exception($decoded_body['error']['error_msg'].' '.json_encode($decoded_body['error']['request_params'], (int) $decoded_body['error']['error_code']));
    }
